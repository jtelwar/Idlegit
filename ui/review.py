...
def _collect_review_focusables(
    ...
    "toggle", or "then_run". Up/Down on the left pane navigates this
    list; `block_idx` says which block's files the right pane should
    show.
    ...
    workflow toggle is immediately followed by its per-workflow
    then-run row when present; then-run-after-push rows come last."""
...
def _review_hints(focusables: List[Tuple[int, str, object]],
    ...
    if panel_focus == "left":
        ...
        # Space toggles the staged-for-commit checkbox on the focused
        # file. Hint string mirrors the current state when we know the
        # block; falls back to a neutral "stage / unstage" otherwise.
        space_label = "toggle stage"
        if blocks is not None and 0 <= focus < len(focusables):
            bi = focusables[focus][0]
            if 0 <= bi < len(blocks):
                block = blocks[bi]
                if 0 <= block.file_selected < len(block.files):
